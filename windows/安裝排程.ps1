...
# 播放內容是從 GitHub Pages 抓的，本機這兩個檔只是斷網時的備援，缺了不擋安裝
if (-not (Test-Path $htmlFile)) {
    Write-Host "提醒：找不到本機備份的播放頁（$htmlFile）。" -ForegroundColor Yellow
    Write-Host "正常情況會從網路播放，所以還是可以裝；但第一次開機就沒網路時會沒有備援。" -ForegroundColor Yellow
}
if (-not (Test-Path $dataFile)) {
    Write-Host "提醒：找不到本機備份的設定檔（$dataFile），將改用線上設定。" -ForegroundColor Yellow
}
